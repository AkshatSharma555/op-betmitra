...
const MatchDetailsModal = ({ match, onClose, onRefresh }) => {
  ...
  const handleResultUpdate = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/matches/update-result/${match._id}`, {
        actualWinner,
  ...
  const handleFullEditSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/matches/edit/${match._id}`, {
  ...
  const handleDelete = async () => {
    if(window.confirm("Are you sure you want to delete this match?")) {
      try {
        await axios.delete(`http://localhost:5000/api/matches/delete/${match._id}`);
        onRefresh();
        onClose();
      } catch (error) {
