    ...
    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-6">
      ...
      <div className="space-y-3">
        {leaderboardData.map((player, index) => (
          <div 
            key={index} 
            className="flex items-center justify-between p-3 bg-[#F5F7FA] rounded-xl border border-gray-100 transition-transform hover:scale-105 duration-200"
          >
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 flex items-center justify-center rounded-full font-bold shadow-sm ${
                index === 0 ? 'bg-yellow-400 text-yellow-900' : 
                index === 1 ? 'bg-gray-300 text-gray-800' : 
                index === 2 ? 'bg-orange-300 text-orange-900' : 
                'bg-white text-[#143866]'
              }`}>
                {index + 1}
              </div>
              <div>
                <p className="font-bold text-[#0B2344]">{player.player}</p>
                <p className="text-xs text-gray-500 font-medium">{player.correct} Won - {player.wrong} Lost</p>
              </div>
            </div>
